...
3. **Помощник** — Grok (SpaceXAI) через серверный proxy *или* локальный fallback  
...
- Coach API: Cloudflare Worker → xAI (`grok-4.5`)
...
### Подключить Grok (опционально)

```bash
# 1) API (Cloudflare)
cd coach-api && npm install
npx wrangler login
npx wrangler secret put XAI_API_KEY   # https://console.x.ai
npx wrangler deploy
